...
export const UnsubscribePage = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const res = await notificationService.unsubscribeAll(token)
        setMessage(res.message ?? 'You have been unsubscribed from all notifications.')
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-cyan-50 px-4">
      <Card className="w-full max-w-lg text-center">
        <p className="text-sm font-semibold uppercase tracking-wide text-primary-600">Notification Preferences</p>
